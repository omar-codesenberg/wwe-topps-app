...
function delayedFetchMock(body, delayMs) {
...
describe('paypal/client — [S4] token cache + single-flight refresh', () => {
    test('30 concurrent token fetches collapse to a single network call', async () => {
        ...
        const results = await Promise.all(Array.from({ length: 30 }, () => client._getAccessTokenForTests()));
    ...
    test('after cache expires, a refresh happens — and is itself single-flighted', async () => {
        let body = { access_token: 't3', expires_in: 32400 };
        ...
        const burst = await Promise.all(Array.from({ length: 20 }, () => client._getAccessTokenForTests()));
...
//# sourceMappingURL=tokenSingleFlight.test.js.map