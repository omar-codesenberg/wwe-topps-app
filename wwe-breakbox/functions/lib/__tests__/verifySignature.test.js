...
const mockedRequest = client_1.request;
...
function makeHeaders(transmissionId = 'TX-1') {
    return {
        transmissionId,
...
beforeEach(() => {
    (0, verifySignature_1._clearVerifierCacheForTests)();
...
describe('verifyWebhookSignature — [C8] verifier endpoint, [S3] tri-state result', () => {
    test('SUCCESS verifier response → { state: "success" }', async () => {
        ...
        const result = await (0, verifySignature_1.verifyWebhookSignature)(WEBHOOK_ID, makeHeaders(), RAW_BODY, CORRELATION_ID);
        ...
        expect(mockedLogger.info).toHaveBeenCalledWith('paypal_webhook_verifier_ok', expect.objectContaining({ correlationId: CORRELATION_ID }));
    });
    test('FAILURE verifier response → { state: "failure" } and emits failure metric', async () => {
        ...
        const result = await (0, verifySignature_1.verifyWebhookSignature)(WEBHOOK_ID, makeHeaders('TX-failure'), RAW_BODY, CORRELATION_ID);
        expect(result.state).toBe('failure');
        expect(result.reason).toBeDefined();
        expect(mockedLogger.error).toHaveBeenCalledWith('paypal_webhook_verifier_failure_response_count', null, expect.objectContaining({
    ...
    test('verifier 5xx (503) → { state: "verifier_unavailable" } and emits call-error metric', async () => {
        ...
        const result = await (0, verifySignature_1.verifyWebhookSignature)(WEBHOOK_ID, makeHeaders('TX-503'), RAW_BODY, CORRELATION_ID);
        expect(result.state).toBe('verifier_unavailable');
        expect(result.reason).toBeDefined();
        expect(mockedLogger.error).toHaveBeenCalledWith('paypal_webhook_verifier_call_error', err, expect.objectContaining({
            correlationId: CORRELATION_ID,
    ...
    test('verifier rate-limited (429) → { state: "verifier_unavailable" }', async () => {
        ...
        mockedRequest.mockRejectedValueOnce(err);
        const result = await (0, verifySignature_1.verifyWebhookSignature)(WEBHOOK_ID, makeHeaders('TX-429'), RAW_BODY, CORRELATION_ID);
        expect(result.state).toBe('verifier_unavailable');
        expect(mockedLogger.error).toHaveBeenCalledWith('paypal_webhook_verifier_call_error', err, expect.objectContaining({ status: 429 }));
    });
    test('network error (no status) → { state: "verifier_unavailable" }', async () => {
        const err = new Error('ECONNREFUSED');
        mockedRequest.mockRejectedValueOnce(err);
        const result = await (0, verifySignature_1.verifyWebhookSignature)(WEBHOOK_ID, makeHeaders('TX-net'), RAW_BODY, CORRELATION_ID);
        expect(result.state).toBe('verifier_unavailable');
...
describe('verifyWebhookSignature — dedupe cache by transmissionId', () => {
    test('cache hit: two calls with same transmissionId invoke request exactly once', async () => {
        ...
        const first = await (0, verifySignature_1.verifyWebhookSignature)(WEBHOOK_ID, headers, RAW_BODY, CORRELATION_ID);
        const second = await (0, verifySignature_1.verifyWebhookSignature)(WEBHOOK_ID, headers, RAW_BODY, CORRELATION_ID);
    ...
    test('cache miss: two calls with different transmissionIds both invoke request', async () => {
        mockedRequest
            ...
            .mockResolvedValueOnce({ verification_status: 'SUCCESS' });
        const a = await (0, verifySignature_1.verifyWebhookSignature)(WEBHOOK_ID, makeHeaders('TX-a'), RAW_BODY, CORRELATION_ID);
        const b = await (0, verifySignature_1.verifyWebhookSignature)(WEBHOOK_ID, makeHeaders('TX-b'), RAW_BODY, CORRELATION_ID);
    ...
    test('_clearVerifierCacheForTests() resets cache so next call invokes request', async () => {
        ...
        await (0, verifySignature_1.verifyWebhookSignature)(WEBHOOK_ID, headers, RAW_BODY, CORRELATION_ID);
        expect(mockedRequest).toHaveBeenCalledTimes(1);
        (0, verifySignature_1._clearVerifierCacheForTests)();
        await (0, verifySignature_1.verifyWebhookSignature)(WEBHOOK_ID, headers, RAW_BODY, CORRELATION_ID);
        expect(mockedRequest).toHaveBeenCalledTimes(2);
