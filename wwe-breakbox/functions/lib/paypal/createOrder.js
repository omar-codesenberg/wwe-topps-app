...
exports._createPayPalOrderImpl = _createPayPalOrderImpl;
...
// ---------------------------------------------------------------------------
async function _createPayPalOrderImpl(data, context) {
    ...
    const eventId = typeof (data === null || data === void 0 ? void 0 : data.eventId) === 'string' ? data.eventId : '';
    const slotId = typeof (data === null || data === void 0 ? void 0 : data.slotId) === 'string' ? data.slotId : '';
    if (!eventId || !slotId) {
    ...
    if (customId.length > CUSTOM_ID_MAX_LEN) {
        throw new functions.https.HttpsError('invalid-argument', `custom_id exceeds ${CUSTOM_ID_MAX_LEN} chars`);
    }
    const slotRef = admin_1.db
        ...
        .collection('slots')
        .doc(slotId);
    const paypalOrdersCol = admin_1.db.collection('paypalOrders');
    // 2) Lock-ownership check.
    const slotSnap = await slotRef.get();
    if (!slotSnap.exists) {
        throw new functions.https.HttpsError('failed-precondition', 'Slot not found');
    }
    const slot = slotSnap.data();
    if (slot.lockedBy !== uid) {
        throw new functions.https.HttpsError('failed-precondition', 'Slot is not locked by caller');
    }
    const lockedUntilMillis = typeof ((_a = slot.lockedUntil) === null || _a === void 0 ? void 0 : _a.toMillis) === 'function'
        ? slot.lockedUntil.toMillis()
        : (_d = (_c = (_b = slot.lockedUntil) === null || _b === void 0 ? void 0 : _b.toDate) === null || _c === void 0 ? void 0 : _c.call(_b).getTime()) !== null && _d !== void 0 ? _d : 0;
    const now = Date.now();
    ...
    if (typeof priceCents !== 'number' ||
        ...
        priceCents <= 0) {
        throw new functions.https.HttpsError('failed-precondition', 'Slot priceCents missing or invalid');
    }
    ...
    if (!existingForSlotSnap.empty) {
        ...
        return {
            orderId: (_e = existing.orderId) !== null && _e !== void 0 ? _e : existingDoc.id,
            approveUrl: (_f = existing.approveUrl) !== null && _f !== void 0 ? _f : '',
            correlationId: (_g = existing.correlationId) !== null && _g !== void 0 ? _g : '',
            alreadyOpen: true,
        };
    }
        ...
        if (newLockedUntilMs > lockedUntilMillis) {
            try {
                await slotRef.update({
                    lockedUntil: admin_1.Timestamp.fromDate(new Date(newLockedUntilMs)),
                });
            }
            catch (err) {
    ...
    try {
        await admin_1.db
            ...
            .set({ count: admin_1.FieldValue.increment(1) }, { merge: true });
    }
    catch (err) {
    ...
    try {
        ...
        paypalResp = await request('POST', '/v2/checkout/orders', orderBody, {
            requestId,
            correlationId,
            project: (r) => {
                const resp = r;
    ...
    if (!paypalResp.id) {
        ...
        throw new functions.https.HttpsError('internal', 'PayPal returned no order id');
    }
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Failed to persist PayPal order');
    }
...
exports.createPayPalOrder = functions
    ...
    .https.onCall((data, context) => _createPayPalOrderImpl(data, context));
