...
const SAFE_ERROR_FIELDS: readonly string[] = [
  'message',
  'name',
  'stack',
  'status',
...
export function info(event: string, payload?: Record<string, unknown>): void {
...
export function error(
  ...
  const extra = sanitizePayload(extraPayload) ?? {};
  ...
  logger.error(event, { ...extra, error: errorPart });
}
 ...
 */
export function withCorrelationId(correlationId: string): ScopedLogger {
  return {
    info(event: string, payload?: Record<string, unknown>): void {
      info(event, { ...(payload ?? {}), correlationId });
    },
    warn(event: string, payload?: Record<string, unknown>): void {
      warn(event, { ...(payload ?? {}), correlationId });
    },
    error(
      event: string,
      errOrPayload?: unknown,
      ...
      error(event, errOrPayload, { ...(extraPayload ?? {}), correlationId });
    },
  };
}

// ---------------------------------------------------------------------------
// [S8] Allow-list projection
// ---------------------------------------------------------------------------
...
function parsePath(path: string): PathSegment[] {
  ...
  for (const part of parts) {
    ...
    if (name.length === 0 && indices.length === 0) continue;
    if (name.length > 0) {
      ...
      for (const idx of indices) segments.push({ key: '', index: idx });
    }
  }
  return segments;
}
...
function resolvePath(value: unknown, segments: readonly PathSegment[]): unknown {
...
function descendOrCreate(
  ...
  if (seg.index !== undefined) {
    if (!Array.isArray(here)) {
      ...
      const arr: unknown[] = [];
      if (seg.key.length > 0) {
        (cursor as Record<string, unknown>)[seg.key] = arr;
      }
      here = arr;
    }
    const arr = here as unknown[];
  ...
  if (Array.isArray(here) || isPlainObject(here)) {
    return here as Record<string, unknown> | unknown[];
  }
  ...
  if (seg.index !== undefined && seg.key.length > 0) {
    const arr = (cursor as Record<string, unknown>)[seg.key] as unknown[];
    arr[seg.index] = replacement;
  } else if (seg.key.length > 0) {
    (cursor as Record<string, unknown>)[seg.key] = replacement;
  }
  return replacement;
}
...
function placeLeaf(
  ...
  if (seg.key.length > 0 && seg.index === undefined) {
    (cursor as Record<string, unknown>)[seg.key] = leaf;
    return;
  }
  if (seg.key.length > 0 && seg.index !== undefined) {
    const obj = cursor as Record<string, unknown>;
    if (!Array.isArray(obj[seg.key])) obj[seg.key] = [];
    const arr = obj[seg.key] as unknown[];
    arr[seg.index] = leaf;
    return;
  }
  if (seg.key.length === 0 && seg.index !== undefined) {
 ...
 * the output).
 */
export function project<T = unknown>(value: unknown, allowList: readonly string[]): T {
  const out: Record<string, unknown> = {};
